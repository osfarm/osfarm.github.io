...
`/fr/communs/comparer/?fiches=<id>,<id>[,<id>]` puts two or three directory entries side by side (licence and its nature, activity state, forge, demo, technology, ecosystem, provenance). **The address is the source of truth**; the selection kept in `localStorage["osfarm-comparateur"]` only helps get there. `docs/fr/comparer.html` embeds the published entries (same `radar-fiches.html` as the directory) in a JSON island read by `assets/js/comparateur.js`; `data/communs.json` is untouched. Cards get a « Comparer » button and the directory a sticky selection bar (`assets/js/comparateur-selection.js`). Every setting — max, licence classes, activity thresholds, popularity units per source, the home-page example — lives in `_data/comparateur.yml`. Entry ids must match `[a-z0-9-]+` (checked by `_annuaire/verifier.py`, with the example ids). The assistant may give a comparison link: the socle describes its form, and the n8n node « Extraire la réponse » keeps it only if every id is a socle entry. Requirements, design and results: `_annuaire/COMPARATEUR.md`.
